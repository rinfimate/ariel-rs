...
/// Render the CSS `<style>` block for an architecture diagram.
...
pub fn group_icon_and_label(
    ...
    format!(
        "<g><g transform=\"translate({ix}, {iy})\"><g><svg xmlns=\"http://www.w3.org/2000/svg\" width=\"30\" height=\"30\" viewBox=\"0 0 80 80\"><g>{inner}</g></svg></g></g><g dy=\"1em\" alignment-baseline=\"middle\" dominant-baseline=\"start\" text-anchor=\"start\" transform=\"translate({tx}, {ty})\"><g><rect class=\"background\" style=\"stroke: none\"></rect><text y=\"-10.1\" style=\"\"><tspan class=\"text-outer-tspan row\" x=\"0\" y=\"-0.1em\" dy=\"1.1em\"><tspan font-style=\"normal\" class=\"text-inner-tspan\" font-weight=\"normal\">{text}</tspan></tspan></text></g></g></g>",
        ix = icon_x, iy = icon_y, inner = inner,
        tx = label_tx, ty = label_ty, text = text,
    )
}

/// Render the plain text label for a group that has no icon.
pub fn group_plain_label(tx: f64, ty: f64, text: &str) -> String {
    format!(
...
pub fn service_label(text: &str) -> String {
    format!(
...
pub fn edge_path(path: &str, lhs: &str, rhs: &str) -> String {
    format!(
...
pub fn edge_arrow(pts: &str, t: &str) -> String {
    format!(
...
pub fn edge_label(x: f64, y: f64, text: &str) -> String {
    format!(
